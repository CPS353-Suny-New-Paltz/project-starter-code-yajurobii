...
public class Checkpoint3TestSuite {
  ...
  public void checkTestImplExists() throws Exception {
    ...
    Assertions.assertTrue(
        allTestTypes.stream()
            .anyMatch(clazz -> !clazz.isInterface() && interfaceType.isAssignableFrom(clazz.getActualClass())),
  ...
  public void checkIntegrationTestStructure() throws Exception {
    Filter<CtExecutableReference<?>> api1Real = (var) -> {
      return var.getDeclaringType().getActualClass().equals(getApiImplClass(NetworkAPI.class)) && var.isConstructor();
    };
    ...
    CtType<?> testDataStoreClass = allTestTypes.stream()
        .filter(clazz -> !clazz.isInterface() && interfaceType.isAssignableFrom(clazz.getActualClass())).findFirst()
        .get();
    Filter<CtExecutableReference<?>> api2Test = (var) -> {
      return var.getDeclaringType().getActualClass().equals(testDataStoreClass.getActualClass()) && var.isConstructor();
    };

    Assertions.assertTrue(
        allTestTypes.stream()
            .anyMatch(clazz -> !clazz.getElements(api1Real).isEmpty() && !clazz.getElements(api3Real).isEmpty()
